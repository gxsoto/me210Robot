...
uint8_t back2SensorTrig = 44;
uint8_t back2SensorEcho = 45; 
...
float back_orienting_thresh = 30.0;
...
void motorForward_for_AlignToLine(uint8_t pin1, uint8_t pin2, uint8_t enable) {
  digitalWrite(pin1, HIGH);
  digitalWrite(pin2, LOW);
...
void slowTurnLeft() {
  // adjusted from 30 to 40 to have less aggressive turn left so it can do line folloiwng 
  analogWrite(pwmMotor1, 80);
  digitalWrite(motor1in1, LOW);
  digitalWrite(motor1in2, HIGH);

  //tune 90...
  analogWrite(pwmMotor2, 170);
...
float readUltrasonicSensor(uint8_t trig, uint8_t echo) {
  ...
  if (dist < 2) return -1;  // reject impossible values
...
void handleOrienting(void) {
  ...
  float back2Dist = readUltrasonicSensor(back2SensorTrig, back2SensorEcho);
  ...
  Serial.print(backDist);
  Serial.print("back2Dist: ");
  Serial.println(back2Dist);
  ...
  // if (rightDist <= right_orienting_thresh && backDist <= back_orienting_thresh && backDist >= 0) {
  if (rightDist <= right_orienting_thresh && backDist <= back_orienting_thresh && abs(backDist - back2Dist) <= 4.5 ) {
    stopMotors();
    if (rightDist == -1.00 && backDist == -1.00 && back2Dist == -1.00) {
      Serial.println("incorrect values -- stuck");
    }
    else {
      Serial.println("oriented (both sensors are under threshold)");
...
void handleMoveToLine(void) {
  ...
  float read_back2Ultra = readUltrasonicSensor(back2SensorTrig, back2SensorEcho);
  float read_rightUltra = readUltrasonicSensor(rightSensorTrig, rightSensorEcho);

  // adjust the backUltra 
  //if (read_backUltra >= 42.0 && abs(read_backUltra - read_back2Ultra <= 3.0) && read_rightUltra <= 15.0) {
  if (read_backUltra >= 49 && abs(read_backUltra - read_back2Ultra) <= 3.0) {
    Serial.println("exited box (it has reached a black line)");
    moveBackward();
    delay(250); 
    currState = ALIGN_TO_LINE;
  }
...
void handleAlignToLine(void) {
  ...
  if (prev_state != ALIGN_TO_LINE) {
    Serial.println("starting slow turn");
  ...
  // if (ir[0] > 200) {
  //   stopMotors();
  //   Serial.println("left IR sensor has seen black line...");
  //   currState = CHECK_BEFORE_LINE_FOLLOWING;

  // }
  /* REPLACING WITH SECTION ABOVE */
  // check if center IR sensors are reading a black line
  // if (ir[1] > 100 || ir[2] > 100) {
  if (ir[1] > 200 || ir[2] > 200) {
    //stopMotors();
    moveBackward();
    delay(250);
    Serial.println("center IR sensors have seen black line");
    currState = LINE_FOLLOWING;
  }
}

// void handleCheckBeforeLineFollowing(void) {
//   Serial.println("Entered Check before Line Following");
//   delay(500);
...
float Kp = 0.3; // tune during testing
uint8_t baseSpeed = 160; // tune during testing

void handleLineFollowing(void) {
  //Serial.println("Entered Line Following Stage...");
...
void handleReleasePuck(void) {
  // if (prev_state != RELEASE_PUCK) {
    ...
    Serial.println("Successfully released puck!");
...
void setup() {
  ...
  pinMode(back2SensorTrig, OUTPUT);
  pinMode(back2SensorEcho, INPUT);
...
void loop() {
  ...
  if (millis() - testingTime >= 300) {
  float back_ultra = readUltrasonicSensor(backSensorTrig, backSensorEcho);
  float back2_ultra = readUltrasonicSensor(back2SensorTrig, back2SensorEcho);
  float right_ultra = readUltrasonicSensor(rightSensorTrig, rightSensorEcho);

  Serial.print("back ultra: ");
  Serial.print(back_ultra);
  Serial.print("back2: ");
  Serial.print(back2_ultra);
  Serial.print("right: ");
  Serial.print(right_ultra);
  Serial.println(" | ");
